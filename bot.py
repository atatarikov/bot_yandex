...
async def city_start_set(message: types.Message):
    markup = types.reply_keyboard.ReplyKeyboardMarkup(row_width=2, resize_keyboard=True)
    btn1 = types.KeyboardButton('Меню')
    markup.add(btn1)
    text = 'Введите название города'
    await message.answer(text, reply_markup=markup)
    await SetCity.waiting_city.set()

@dp.message_handler(state=SetCity.waiting_city)
async def city_set(message: types.Message, state: FSMContext):
    ...
    await message.answer(city.get('waiting_city')+'jjj', reply_markup=markup)
    await state.finish()
