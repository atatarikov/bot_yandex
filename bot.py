...
async def get_user_city_weather(message: types.Message):
    ...
    ndata = data['fact']
    orm.create_report(message.from_user.id, data["temp"], ndata["feels_like"], ndata["wind_speed"], ndata["pressure_mm"], city)
    text = f'Погода в {city}\nТемпература: {ndata["temp"]} C\nОщущается как: {ndata["feels_like"]} C \nСкорость ветра: {ndata["wind_speed"]}м/с\nДавление: {ndata["pressure_mm"]}мм'
    await message.answer(text, reply_markup=markup)

@dp.message_handler(regexp='Меню')
async def start_message1(message: types.Message):
    markup = types.reply_keyboard.ReplyKeyboardMarkup(row_width=2)
    btn1 = types.KeyboardButton('Погода в моём городе')
    btn2 = types.KeyboardButton('Погода в другом месте')
    btn3 = types.KeyboardButton('История')
...
async def city_chosen(message: types.Message, state: FSMContext):
    ...
    ndata = data['fact']
    orm.create_report(message.from_user.id, ndata["temp"], ndata["feels_like"], ndata["wind_speed"], ndata["pressure_mm"], city.get('waiting_city'))
    ndata = data['fact']
